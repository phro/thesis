...
\begin{definition}[meta-objects]
        ...
        monoidal category $\CC$ (for example, a monoid). We say $\CC$ is a
        \defi{meta-structure} (for example, a \defi{meta-monoid}) if all the
        objects of $\CC$ are equal to $A_S$ for some finite set $S$, and all
...
\begin{definition}[monoid]
        A \defi{monoid} is an object $A\in\CC$ in a monoidal category together
        with an associative multiplication $\map {m^{i,j}_{k}} {A_{\set{i,j}}}
        {A_{\set{k}}}$, and a multiplicative unit
        $\map{\unit_{1}}{A_{\emptyset}}{A_{\set{i}}}$.\footnote{
