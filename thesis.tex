...
Tangled objects also have the structure of a meta-Hopf algebra.

\begin{definition}[pure tangle]
        A \defi{pure tangle} is an embedding of line segments (called
        \defi{strands}) into the thickened unit disk $D \times [-1,1]$ such that
...
\begin{theorem}[pure tangles are planar diagrams]\label{thm:pure_tangle}
        The set of pure tangles on $n$ strands is realised combinatorially as
        the set of planar tangle diagrams in a disc, all of whose strands
        connect to the boundary circle.
\end{theorem}
\begin{proof}
        %TODO
\end{proof}

We extend this definition by dropping the planarity condition.
\begin{definition}[virtual tangles]
        A \defi{virtual tangle} is a diagram satisfying all conditions laid out
        in \cref{thm:pure_tangle} except planarity.
\end{definition}

\begin{theorem}[virtual tangles are just crossings]
        The data of a virtual tangle can be recovered from just the collection
        of crossings and the order in which each strand encounters them.
\end{theorem}
\begin{proof}
        %TODO
\end{proof}
...
\begin{definition}[traced meta-algebra]
        ...
        \begin{equation}
                \map {\mult^{i,j}_k}
                        ...
                        {A_{\set{k}\sqcup S, L}} 
        \end{equation}
        for $i,j$ disjoint from both $S$ and $L$.
