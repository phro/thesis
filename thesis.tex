...
\begin{definition}[concatenation in a meta-monoidal category] % todo: choose 1
        ...
        be pairwise disjoint finite sets, and $ϕ^{D_1}_{C_1\sqcup S}$ and
        $ψ^{D_2\sqcup S}_{C_2}$ be morphisms in $\msc C$. We define their
        concatenation by:\footnote{To better complement reading direction, we
        use the left-to-right composition symbol $f \then g \defeq g \circ f$.}
...
^{D_1 \cup (D_2 \setminus C_1) \cup (D_3 \setminus (C_2 \cup (C_1 \setminus D_2)))}
_{(C_1 \setminus D_2) \cup C_2 \cup (D_3 \setminus C_2)}\\
                &\then
                \pn*{
                        (ϕ_3)^{D_3}_{C_3}
                        \otimes \id
