...
\begin{definition}[meta-monoidal category]
        ...
        \begin{itemize}
                \item Objects are sets $\msc C_S$ indexed by finite sets $S$.
                \item Given a morphism $ϕ$ in $\msc C$, its domain and codomain
                        are thereby completely determined by two finite sets,
                        say $D$ and $S$, respectively. Such morphisms will be
                        denoted by $ϕ^D_C$.
...
\begin{definition}[algebra]
        An \defi{algebra} is a vector space $A$ over a field $k$ together with
        an associative multiplication $\map {m} {A \otimes A} {A}$, and a
        multiplicative unit $\one\in A$. It is more natural to think of the unit
        as a  map $\map{\unit}{k}{A}$ by linearly extending the assignment
        $\unit(1) = \one$.
\end{definition}
