...
\begin{enumerate}
        ...
        \item Instead of implicitly including extensions of morphisms to higher
                ...
                {G_{\set{i,j}}} {G_{\set{k}}}$\footnote{This notation is defined in
                %todo
...
\begin{theorem}[virtual tangles form a meta-Hopf algebra]
        The collection $\tangle_S$ forms a meta-Hopf algebra with the following
        operations:
        \begin{itemize}
                \item multiplication $\mult^{ij}_{k}[S]$ takes a tangle with
                        strands $S\sqcup\set{i,j}$ and glues the end of strand
                        $i$ to strand $j$, labelling the resulting strand $k$.
                \item the unit $\unit_{i}[S]$ takes a tangle diagram with
                        strands $S$ and introduces a new strand $i$ which does
                        not touch any of the other strands.
                \item the comultiplication $\comult^{i}_{jk}[S]$ takes a tangle
                        with strands $S\sqcup \set{i}$ and doubles strand $i$,
