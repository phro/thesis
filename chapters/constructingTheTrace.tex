...
equivalent to no two $\yo^n \ao^k \xo^n$'s with distinct choices of exponents
being related under a sequence of relations. Since \cref{lem:coinvLieAlg} allows
us to only consider sequences of relations of the form $\ad_\zo$ for $\zo$ a
one-letter word in $\set{\yo, \bo, \ao, \xo}$, we know that
$\liebk{\Alg}{\CU}
= \ad_{\yo}\CU + \ad_{\bo}\CU + \ad_{\ao}\CU + \ad_{\xo}\CU
= \ad_{\yo}\CU + \ad_{\ao}\CU$.

Observe:
\begin{equation}
        \begin{multlined}[t]
                {\ad_{\ao}\CU} + {\ad_{\yo}\CU} = \Span\pn[\Big]{
                        \setbuilder*{\yo^n \bo^m \ao^l \xo^r}{n \neq r}
                        + \\
                        \setbuilder*{
                                (n+1)\yo^n \bo^m \ao^k \xo^n
                                - \yo^{n+1}\bo^{m-1}\pn[\big]{\ao^k-(\ao-1)^k}\xo^{n+1}
                        }{n \ge 0, m \ge 1}
                }
        \end{multlined}
\end{equation}
...
$\set{\yo^n \ao^k \xo^n}_{n, k\ge 0}$ follows.
\end{proof}
