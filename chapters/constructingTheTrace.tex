...
\begin{definition}[traced meta-algebra]
        ...
        \begin{enumerate}
                ...
                \item For each pair of disjoint finite sets $S$, $L$ and index
                        ...
                        meta-algebras in the following way: \footnote{%
                                This is the \defi{cyclic} property of the
                                trace.%
                        }
                        \begin{equation}
...
\begin{remark}
        \Cref{thm:trace_gaussian} requires that the coefficient of the $ab$-term
        in the exponential is $0$. This restriction allows the computed value to
        be a composition of elementary functions. Expanding the computation to
        include more general expressions is not necessary when applying a trace
        to an open tangle, since the $a_ib_i$-coefficient associated to an open
        tangle is exactly the self-linking number of component $i$, though when
        applying \cref{eq:trace_on_gaussian} to a more than open component, the
        author was not able to find a workable closed form for the trace.
\end{remark}
