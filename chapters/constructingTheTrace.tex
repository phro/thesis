...
\begin{definition}[Impure Upright Tangles]
        Let $\RVT_{L, S}$ be the set of upright tangles with open
...
\begin{proof}
        ...
        \cref{thm:tangles_meta_algebra}, so $\RVT_{\emptyset, S} = \tangle_S$ is a
...
Since all relations are accounted for, setting $m=0$ demonstrates this set is
linearly independent, and we have a basis.
\end{proof}
