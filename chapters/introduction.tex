...
Given any monoid object $M$ in a monoidal category $\CC$, $M$ has the structure
of a meta-monoid $\set{M_X}_X$ via $M_X \defeq M^{\otimes X}$, $\mult^{ij}_k[X]
\defeq \mult^{ij}_k \otimes \Id{X}$, and $\unit_i[X](v) \defeq 1_i \otimes v$
for any $v\in M^{\otimes X}$.
...
$\map {\mult^{ij}_{k}}
        ...
        {M_{X\sqcup\set{k}}}$
by
$\mult^{i, j}_{k}\pn*{(a_{rs})_{rs}} \defeq
...
\begin{definition}[meta-object]
        ...
        \begin{enumerate}
                \item A collection of objects $A_X$, one for each choice of
                        finite set $X$. (This serves as the analogue to monoidal
                        powers.)
                \item A collection of \defi{operations} $ϕ_1, ϕ_2, \dots, ϕ_n$
                        each with a \defi{signature} $\signature{ϕ_i}\in \Z_{\ge
                        0} \times \Z_{\ge 0}$. For any pair of finite sets
                        $(D, C)$ satisfying $(\ord D, \ord C) = \signature{ϕ}$,
                        we have a morphism:
                        \begin{equation}
                              \map {ϕ^D_C} {A_D} {A_C}
                        \end{equation}
        ...
        $\map {ι_f} {A_X} [\toiso] {A_Y}$.

        Finally, for each operation $ϕ^{D}_{C}$, there is a collection of
...
\begin{definition}[upright pure tangle diagram]
        ...
        called \defi{upright tangle diagrams}.
\end{definition}
\begin{remark}
