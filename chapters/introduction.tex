...
vector may equivalently be written as $u_iv_jw_k = v_ju_iw_k = w_kv_ju_i \in
...
$T[S] \defeq T^{D}_{C} \Id{S}$ for this concept. When no ambiguity arises, we
will also suppress the \enquote{$[S]$} so that $T^D_C$ represents more
generally:
\begin{equation}
...
of an \defi{algebra}. Then $A_\emptyset$ is the field $\K$. It is more common to
...
\begin{definition}[open tangle]
        ...
        isotopy of the embeddings which fixes the endpoints of the strands. The
...
which one may think of as open tangles with the strings replaced with thin
ribbons.
...
\begin{definition}[upright open tangle diagram]
        An \defi{upright tangle diagram} is a tangle diagram with the further
...
\begin{enumerate}
        \item We show $\tangle X$ is a meta-algebra:
                \begin{enumerate}
                        \item To establish associativity (\cref{eq:cd_mult}),
                                notice that when stitching a sequence of strands
                                together in a fixed order, the order in which
                                the individual stitches are made does not affect
                                the resulting strand; the resulting strand has
                                the same endpoints and follows the same path.
...
\begin{definition}[morphism of meta-objects]
        ...
        \defi{morphism} $Φ$ between these meta-objects is a map $\map {Φ}
