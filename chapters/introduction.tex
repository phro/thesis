...
Next, we define the bialgebra structure of $\CU$. For any $z\in\set{y, b, a,
x}$, we have
\begin{align}
        \comult_{i,j}(y) &= y_iB_j + y_j\\
        \comult_{i,j}(b) &= b_i + b_j\\
        \comult_{i,j}(a) &= a_i + a_j\\
        \comult_{i,j}(x) &= x_i + x_j\\
        \counit(z) &= 0
\end{align}
...
the spinner $\spin$:
\begin{align}
        \Rmat_{i,j} &\defeq \exp\pn*{a_jb_i + \frac{1-B_i}{b_i}y_ix_j}\\
        \spin &\defeq \sqrt{B}
\end{align}
