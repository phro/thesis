...
pair size = (5,8);
path disk = shift(0,-1)*scale(size.x,1+size.y)*unitsquare;
...
xing(((flip*a[0]).x,0.5),a[1]);
drawTangleArc(flip*a[4]{S}::{W}(3+(size.x-startx-2/3)/2,-0.25)..{N}(startx,0.5),true,0.2);
drawTangleArc(((flip*a[0]).x,-1)--((flip*a[0]).x,0.5));
drawTangleEndpoint(((flip*a[0]).x,-1));

real frontHeight = 2.5;

drawFrontArc((0,frontHeight){E}..(2.25,3)..{N}(2.5,4.5)..{S}(3.7,4.5)::(3.7,3)::{E}(size.x,frontHeight),true);
drawFrontArc((0,frontHeight){E}--(size.x,frontHeight));
