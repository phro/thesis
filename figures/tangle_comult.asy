...
drawTangleArc(rhs*subpath(q,0,0.5));
drawTangleArc(rhs*subpath(p1,1,2));
drawTangleArc(rhs*subpath(p2,1,2));
drawTangleArc(rhs*subpath(p2,0,1),"$k$");
drawTangleArc(rhs*subpath(p1,0,1),"$j$");
drawTangleArc(rhs*subpath(q,0.5,1));
dot(rhs*(0,0),tangleEndpoint);
dot(rhs*(0,2),tangleEndpoint);
dot(rhs*shift(eps)*(0,0),tangleEndpoint);
dot(rhs*shift(eps)*(0,2),tangleEndpoint);
dot(rhs*(-0.7,0),tangleEndpoint);
dot(rhs*(-0.7,2),tangleEndpoint);

draw((0.8,1)--(1.2,1),boundaryArc,arrow=EndArrow(arrowhead=TeXHead),
        L=Label("$\Delta^{i}_{jk}$",Relative(0.4),LeftSide));
